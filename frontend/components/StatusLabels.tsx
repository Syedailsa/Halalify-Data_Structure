...
const LABELS = [
  { name: "Halal",     nameBg: "#f0fdf4", border: "#86efac", titleColor: "#14532d", descColor: "#166534", desc: "Verified safe and permissible for consumption according to Islamic law",         sealRing: "#16a34a", sealFill: "rgba(34,197,94,0.08)",  arabic: "حلال", sub: "100% HALAL PRODUCTS" },
  { name: "Haram",     nameBg: "#fff1f2", border: "#fecaca", titleColor: "#7f1d1d", descColor: "#991b1b", desc: "Contains prohibited ingredients or substances not allowed in Islam",            sealRing: "#dc2626", sealFill: "rgba(239,68,68,0.07)",  arabic: "حرام", sub: "100% HARAM PRODUCTS" },
  { name: "Mashbooh",  nameBg: "#fffbeb", border: "#fed7aa", titleColor: "#78350f", descColor: "#92400e", desc: "Doubtful status — requires verification from local certifying bodies",          sealRing: "#ea580c", sealFill: "rgba(249,115,22,0.07)", arabic: "مشبوه", sub: "100% MASHBOOH"      },
];
...
function Seal({ label }: { label: typeof LABELS[0] }) {
  ...
  return (
    <svg viewBox="0 0 130 130" style={{ width: 110, height: 110, flexShrink: 0 }}>
      <circle cx="65" cy="65" r="61" fill="none" stroke={r} strokeWidth="1.5" opacity="0.3"/>
      {Array.from({length:32}).map((_,i) => { const a=(i*360/32)*Math.PI/180; return <line key={i} x1={65+57*Math.cos(a)} y1={65+57*Math.sin(a)} x2={65+62*Math.cos(a)} y2={65+62*Math.sin(a)} stroke={r} strokeWidth="1.5" opacity="0.45"/>; })}
      <circle cx="65" cy="65" r="53" fill="none" stroke={r} strokeWidth="1.5" strokeDasharray="3 3" opacity="0.4"/>
      <circle cx="65" cy="65" r="48" fill={label.sealFill}/>
      <circle cx="65" cy="65" r="48" fill="none" stroke={r} strokeWidth="1" opacity="0.25"/>
      <text x="65" y="60" textAnchor="middle" fill={r} fontSize="20" fontWeight="700" fontFamily="serif">{label.arabic}</text>
...
export function StatusLabels() {
  ...
  return (
    <section ref={ref} className="status-section" style={{ padding: "clamp(3rem,6vw,5rem) 0" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 clamp(1.25rem,4vw,2.5rem)" }}>
        <div className="reveal" style={{ textAlign: "center", marginBottom: "clamp(2rem,4vw,3.5rem)" }}>
          <span style={{ fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#6b7280", border: "1px solid #e5e7eb", borderRadius: 999, padding: "5px 14px", display: "inline-block", marginBottom: 16 }}>
            Halal Status at a Glance with Clear, Reliable Labels
          </span>
          <h2 style={{ fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 900, color: "#111", letterSpacing: "-0.02em" }}>
            Understanding Status Labels
          </h2>
        </div>

        <div className="status-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem" }}>
          {LABELS.map((l, i) => (
            <div key={l.name} className="status-card reveal" style={{ background: l.nameBg, borderColor: l.border, transitionDelay: `${i*100}ms` }}>
              <Seal label={l} />
              <h3 style={{ fontSize: "1.4rem", fontWeight: 900, color: l.titleColor, marginTop: "1rem", marginBottom: "0.5rem" }}>{l.name}</h3>
              <p style={{ fontSize: "0.88rem", color: "#4b5563", lineHeight: 1.65 }}>{l.desc}</p>
            </div>
          ))}
        </div>
      ...
      <style>{`
        @media (max-width: 860px) {
          .status-grid { grid-template-columns: 1fr !important; max-width: 480px; margin-left: auto; margin-right: auto; }
        }
        @media (max-width: 600px) {
          .status-grid { max-width: 100% !important; }
        }
      `}</style>
    </section>
  );
