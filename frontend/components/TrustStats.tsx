...
const TrophyIcon = ({ color }: { color: string }) => (
  <svg width="60" height="60" viewBox="-50 -45 100 90" fill="none">
    <path d="M-22,-32 L22,-32 L22,-4 Q22,18 0,24 Q-22,18 -22,-4 Z" fill={color}/>
    <path d="M-22,-24 L-34,-24 Q-42,-24 -42,-14 Q-42,-4 -32,-4 L-22,-4" fill="none" stroke={color} strokeWidth="5" strokeLinecap="round"/>
    <path d="M22,-24 L34,-24 Q42,-24 42,-14 Q42,-4 32,-4 L22,-4" fill="none" stroke={color} strokeWidth="5" strokeLinecap="round"/>
    <rect x="-5" y="24" width="10" height="12" fill={color}/>
    <rect x="-18" y="36" width="36" height="7" rx="3.5" fill={color}/>
  </svg>
);
...
const TagIcon = ({ color }: { color: string }) => (
  <svg width="60" height="60" viewBox="-55 -38 100 76" fill="none">
    <path d="M-28,-28 L4,-28 Q12,-28 18,-22 L34,-6 Q40,0 34,6 L18,22 Q12,28 4,28 L-28,28 Q-36,28 -36,20 L-36,-20 Q-36,-28 -28,-28 Z" fill={color}/>
    <circle cx="-20" cy="-16" r="5" fill="#111"/>
    <path d="M-38,-10 Q-46,-10 -46,0 Q-46,10 -38,10" fill="none" stroke={color} strokeWidth="4" strokeLinecap="round" opacity="0.7"/>
  </svg>
);
...
function StatCard({ stat, started, idx }: { stat: typeof STATS[0]; started: boolean; idx: number }) {
  ...
  const displayVal = () => {
    if (stat.end === 0) return stat.display;
    if (stat.suffix === "K+") return `${count}K+`;
    if (stat.suffix === "+") return stat.end >= 10000 ? `${count.toLocaleString()}+` : `${count}+`;
    return stat.display;
  };
  ...
  return (
    <div style={{
      ...
      padding: "clamp(1.4rem,3vw,2rem) clamp(1.25rem,2.5vw,1.75rem)",
      display: "flex", flexDirection: "column", gap: "0.85rem",
      opacity: started ? 1 : 0, transform: started ? "translateY(0)" : "translateY(40px)",
      transition: `opacity 0.6s ease ${idx * 120}ms, transform 0.6s ease ${idx * 120}ms`,
      minHeight: "180px",
    }}>
      <div style={{ display: "inline-flex", alignSelf: "flex-start" }}>
        <span style={{ background: pillBg, color: "#111", borderRadius: "999px", padding: "0.3rem 0.85rem", fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.08em", whiteSpace: "nowrap" }}>
          {stat.label}
        </span>
      </div>
      <div style={{ marginTop: "0.1rem" }}>
        {stat.dark ? <TagIcon color={iconColor} /> : <TrophyIcon color={iconColor} />}
      </div>
      <div style={{ fontSize: "clamp(1.8rem,3.5vw,2.8rem)", fontWeight: 900, color: textColor, letterSpacing: "-0.03em", lineHeight: 1, marginTop: "auto" }}>
        {displayVal()}
      </div>
    </div>
  );
...
export default function TrustStats() {
  ...
  return (
    <section ref={ref} style={{ background: "#f5f5f5", padding: "clamp(3rem,6vw,5rem) 0" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 clamp(1.25rem,4vw,2.5rem)" }}>
        <div style={{ textAlign: "center", marginBottom: "clamp(2rem,4vw,3rem)" }}>
          <h2 style={{ fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 900, color: "#111", letterSpacing: "-0.02em", margin: 0 }}>
            Trusted by{" "}
            <span style={{ color: "rgba(17,17,17,0.18)" }}>Thousands</span>
          </h2>
        </div>

        <div className="trust-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1.25rem" }}>
          {STATS.map((s, i) => (
            <StatCard key={s.label} stat={s} started={started} idx={i} />
          ))}
        </div>
